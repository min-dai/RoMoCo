...
std::vector<int> G1Model::actuated_u_idx(AnkleMotorStatus left_ankle_status, AnkleMotorStatus right_ankle_status) const
{
    ...
    auto add_leg_joints = [&](bool is_left, AnkleMotorStatus ankle_status)
    {
        int hip_pitch = is_left ? 0 : 6;
        int hip_roll = is_left ? 1 : 7;
        int hip_yaw = is_left ? 2 : 8;
...
void G1Model::AddFrames()
{
    ...
    std::string base_joint_name = "waist_pitch_joint"; 
...
Kinematics1D G1Model::GetBaseDeltaPitch()
{
    return (baseB_.kinematics.z() - baseF_.kinematics.z())/0.1;
}
Kinematics1D G1Model::GetBaseDeltaRoll()
{
