#include <memory>
#include <rclcpp/rclcpp.hpp>

#include "biped_ros/ros_interface_node.hpp"

#include "g1_mujoco_interface.hpp"

#include "g1_model_leg.hpp"

#include "biped_ros/ros_load_config.hpp"
int main(int argc, char *argv[])
{
  rclcpp::init(argc, argv);
  bool use_estimation = false;
  if (argc > 1) {
    use_estimation = std::string(argv[1]) == "true" || std::string(argv[1]) == "1";
  }

  RosLoadConfig ros_config("g1_simulation", "config_18dof", "ROBOTLOG/G1");

  std::unique_ptr<RobotBasePinocchio> robot_ptr = std::make_unique<G1ModelLeg>(ros_config.config_folder);

  std::shared_ptr<InterfaceBase> interface;
  
  if (use_estimation)
      interface = std::make_shared<G1MujocoInterface>(ros_config.config_folder, ros_config.log_path, std::move(robot_ptr));
  else
      interface = std::make_shared<G1MujocoInterface>(ros_config.config_folder, ros_config.log_path);

  // Wrap in ROS interface node
  auto node = std::make_shared<RosInterfaceNode>(ros_config.config_folder, ros_config.log_path, std::move(interface));

  // Spin
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
