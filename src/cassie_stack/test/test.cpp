...
TEST_F(CassieTest, TestStandingController)
{
   ...
   std::string config_file = home + "/biped_simulation/src/cassie_stack/config/standing_config.yaml";

   std::string mujoco_config_file = home + "/biped_simulation/src/cassie_stack/config/mujoco_config.yaml";

   YAMLParser yaml_parser(mujoco_config_file);
   std::string urdf_name = yaml_parser.get_string("urdf_name");
   std::string urdf_path = home + "/biped_simulation/src/cassie_stack/cassie_model_files/" + urdf_name;
