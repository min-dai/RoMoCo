...
#include "cassie_interface/cassiemujoco.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <math.h>
...
#include "mjxmacro.h"
#include "uitools.h"
#include "cassie_interface/cassie_core_sim.h"
#include "cassie_interface/state_output.h"
#include "cassie_interface/pd_input.h"
...
// double yaw_offset = 0.2;
//     double qpos_init[] =
//         { 0.0045, yaw_offset, 0.4973, 0.9785, -0.0164, 0.01787, -0.2049,
//          -1.1997, 0, 1.4267, 0, -1.5244, 1.5244, -1.5968,
//          -0.0045, -yaw_offset, 0.4973, 0.9786, 0.00386, -0.01524, -0.2051,
//          -1.1997, 0, 1.4267, 0, -1.5244, 1.5244, -1.5968};
    ...
    for (int i = 0; i < c->d->ncon; ++i) {
        ...
        if (body1 == left_foot_body_id || body2 == left_foot_body_id) {
            ...
            mju_rotVecMatT_fp(force_global, force_torque,
                c->d->contact[i].frame);
            double tmp[3] = {force_torque[3],force_torque[4],force_torque[5]};
            mju_rotVecMatT_fp(torque_global, tmp,
                c->d->contact[i].frame);
            ...
            if (body1 == left_foot_body_id) {
                ...
                for (int j = 0; j < 3; ++j) {
                    cfrc[j] -= force_global[j];
                    cfrc[j + 3] -= torque_global[j];
                }
            }
            else {
                for (int j = 0; j < 3; ++j) {
...
void cassie_vis_foot_ZMP(const cassie_vis_t* c, double pZMP[6]) {
    //Brute force: assume robot is not falled and only capsule 
    mju_zero_fp(pZMP, 6);
    // Zero the pZMP
    bool leftfoot_contact = false;
    bool righfoot_contact = false;
    int leftfoot_connum = 0;
    int righfoot_connum = 0;
    double force_torque[6];
    double force_global[3];

    double leftfoot_force[6];
    double righfoot_force[6];
    double leftfoot_force_pos[6];
    double righfoot_force_pos[6];


    int right_heel_id = mj_name2id_fp(c->m, mjOBJ_SITE, "right-foot-heel");
    int left_heel_id = mj_name2id_fp(c->m, mjOBJ_SITE, "left-foot-heel");


    for (int i = 0; i < c->d->ncon; ++i) {
        // printf("number of contact : %d, i = %d \n", c->d->ncon, i);
        // Get body IDs for both geoms in the collision
        int body1 = c->m->geom_bodyid[c->d->contact[i].geom1];
        int body2 = c->m->geom_bodyid[c->d->contact[i].geom2];
        ...
        if (body1 == left_foot_body_id || body2 == left_foot_body_id) {
            ...
            if (body1 == left_foot_body_id) {
                for (int j = 0; j < 3; ++j) {
                    leftfoot_force[3 * (leftfoot_connum - 1) + j] = -force_global[j];
                }
            }
            else {
                for (int j = 0; j < 3; ++j) {
            ...
            double heelpos[3], conpos[3], conpos_local[3];

            mju_copy_fp(heelpos, &c->d->site_xpos[3 * left_heel_id], 3);

            mju_copy_fp(conpos, c->d->contact[i].pos, 3);
            // mju_copy_fp(footpos, &c->d->xipos[3 * right_foot_body_id], 3);
            // mju_copy_fp(&cpos[3], &c->d->xpos[3 * right_foot_body_id], 3);

            double tmp[3];
            mju_sub3_fp(tmp, conpos, heelpos);
            mju_rotVecMat_fp(conpos_local, tmp, c->d->contact[i].frame);
            for (int i = 0; i < 3;i++) {
                leftfoot_force_pos[3 * (leftfoot_connum - 1) + i] = conpos_local[i];
            }

            // printf("pos =  [%f, %f, %f] \n", c->d->contact[i].pos[0], c->d->contact[i].pos[1], c->d->contact[i].pos[2]);

        }

        // Right foot
        if (body1 == right_foot_body_id || body2 == right_foot_body_id) {
            righfoot_contact = true;
            righfoot_connum++;

            mj_contactForce_fp(c->m, c->d, i, force_torque);
            mju_rotVecMatT_fp(force_global, force_torque, c->d->contact[i].frame);

            if (body1 == right_foot_body_id) {
                for (int j = 0; j < 3; ++j) {
                    righfoot_force[3 * (righfoot_connum - 1) + j] = -force_global[j];
                }
            }
            else {
                for (int j = 0; j < 3; ++j) {
            ...
            mju_copy_fp(heelpos, &c->d->site_xpos[3 * right_heel_id], 3);
            ...
            mju_rotVecMat_fp(conpos_local, tmp, c->d->contact[i].frame);
            for (int i = 0; i < 3;i++) {
                righfoot_force_pos[3 * (righfoot_connum - 1) + i] = conpos_local[i];
            }
            // printf("con pos =  [%f, %f, %f] \n", c->d->contact[i].pos[0], c->d->contact[i].pos[1], c->d->contact[i].pos[2]);
            // printf("heel pos =  [%f, %f, %f] \n", heelpos[0], heelpos[1], heelpos[2]);
            // printf("con pos local=  [%f, %f, %f] \n", conpos_local[0], conpos_local[1], conpos_local[2]);


            // double footCOMpos[3], anklepos[3];
            // mju_copy_fp(footCOMpos, &c->d->xipos[3 * right_foot_body_id], 3);
            // mju_copy_fp(anklepos, &c->d->xpos[3 * right_foot_body_id], 3);

            // printf("heel pos =  [%f, %f, %f] \n", heelpos[0], heelpos[1], heelpos[2]);
            // printf("footCOM pos =  [%f, %f, %f] \n", footCOMpos[0], footCOMpos[1], footCOMpos[2]);
            // printf("ankle pos =  [%f, %f, %f] \n", anklepos[0], anklepos[1], anklepos[2]);
    ...
    // printf("leftfoot_force_pos =  [%f, %f, %f,%f, %f, %f] \n", leftfoot_force_pos[0], leftfoot_force_pos[1], leftfoot_force_pos[2], leftfoot_force_pos[3], leftfoot_force_pos[4], leftfoot_force_pos[5]);
    // printf("leftfoot_force =  [%f, %f, %f,%f, %f, %f] \n", leftfoot_force[0], leftfoot_force[1], leftfoot_force[2], leftfoot_force[3], leftfoot_force[4], leftfoot_force[5]);
    ...
    if (leftfoot_contact) {
        for (int i = 0; i < 3; i++) {
            double den = 0;
            double num = 0;
            for (int j = 0; j < leftfoot_connum;j++) {
                den += leftfoot_force[(j + 1) * 3 - 1]; //Fz
                num += leftfoot_force[(j + 1) * 3 - 1] * leftfoot_force_pos[3 * j + i];
            }
            pZMP[i] = num / den;
        }
    ...
    if (righfoot_contact) {
        for (int i = 0; i < 3; i++) {
            double den = 0;
            double num = 0;
            for (int j = 0; j < righfoot_connum;j++) {
                den += righfoot_force[(j + 1) * 3 - 1]; //Fz
                num += righfoot_force[(j + 1) * 3 - 1] * righfoot_force_pos[3 * j + i];
            }
...
// ZMP for both SS and DS phase
void cassie_vis_foot_ZMP2(const cassie_vis_t* c, double pZMP[6]) {
    //Brute force: assume robot is not falled and only capsule 
    mju_zero_fp(pZMP, 6);
    // Zero the pZMP
    bool leftfoot_contact = false;
    bool rightfoot_contact = false;
    int leftfoot_connum = 0;
    int rightfoot_connum = 0;
    double force_torque[6];
    double force_global[3];

    double leftfoot_force[6];
    double rightfoot_force[6];
    double leftfoot_force_pos_world[6];
    double rightfoot_force_pos_world[6];


    int right_heel_id = mj_name2id_fp(c->m, mjOBJ_SITE, "right-foot-heel");
    int left_heel_id = mj_name2id_fp(c->m, mjOBJ_SITE, "left-foot-heel");
    
    double leftankle_pos[3];
    double rightankle_pos[3];

    mju_copy_fp(leftankle_pos, &c->d->xpos[3 * left_foot_body_id], 3);
    mju_copy_fp(rightankle_pos, &c->d->xpos[3 * right_foot_body_id], 3);
    // printf("left ankle pos =  [%f, %f, %f] \n", leftankle_pos[0], leftankle_pos[1], leftankle_pos[2]);
    // printf("righ ankle pos =  [%f, %f, %f] \n", rightankle_pos[0], rightankle_pos[1], rightankle_pos[2]);
           
    // foot (algined with pelvis) relative to world
    double leftfoot_xmat[9];
    double rightfoot_xmat[9];

    //Agility's unusual foot frame definition: foot frame is not aligned with base frame, we need the final vectors to be in base frame
    double footRbase[9] = {-0.766044443118978,0,-0.642787609686539, 0.642787609686539,0, -0.766044443118978,0,-1.,0};

    mju_copy_fp(leftfoot_xmat, &c->d->site_xmat[9 * left_heel_id], 9);
    mju_copy_fp(rightfoot_xmat, &c->d->site_xmat[9 * right_heel_id], 9);
    ...
    for (int i = 0; i < c->d->ncon; ++i) {
        ...
        if (body1 == left_foot_body_id || body2 == left_foot_body_id) {
            ...
            for (int i = 0; i < 3;i++) {
                leftfoot_force_pos_world[3 * (leftfoot_connum - 1) + i] = conpos[i];
            }
        }

        // Right foot
        if (body1 == right_foot_body_id || body2 == right_foot_body_id) {
            rightfoot_contact = true;
            rightfoot_connum++;
            ...
            if (body1 == right_foot_body_id) {
                for (int j = 0; j < 3; ++j) {
                    rightfoot_force[3 * (rightfoot_connum - 1) + j] = -force_global[j];
                }
            }
            else {
                for (int j = 0; j < 3; ++j) {
            ...
            for (int i = 0; i < 3;i++) {
                rightfoot_force_pos_world[3 * (rightfoot_connum - 1) + i] = conpos[i];
            }
           
            // printf("con pos =  [%f, %f, %f] \n", c->d->contact[i].pos[0], c->d->contact[i].pos[1], c->d->contact[i].pos[2]);
            // printf("heel pos =  [%f, %f, %f] \n", heelpos[0], heelpos[1], heelpos[2]);
            // printf("ankle pos =  [%f, %f, %f] \n", anklepos[0], anklepos[1], anklepos[2]);
            // printf("con pos2 heel =  [%f, %f, %f] \n", tmp[0], tmp[1], tmp[2]);
        ...
        // printf("right force=  [%f, %f, %f,%f, %f, %f] \n", rightfoot_force[0], rightfoot_force[1], rightfoot_force[2], rightfoot_force[3], rightfoot_force[4], rightfoot_force[5]);
        // printf("left force=  [%f, %f, %f,%f, %f, %f] \n", leftfoot_force[0], leftfoot_force[1], leftfoot_force[2], leftfoot_force[3], leftfoot_force[4], leftfoot_force[5]);
    ...
    if (leftfoot_contact && rightfoot_contact){
        for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < rightfoot_connum;j++) {
                den[i] += rightfoot_force[(j + 1) * 3 - 1]; //Fz
                
                double tmp[3], tmp2[3], conpos_local[3];
                mju_sub3_fp(tmp, rightfoot_force_pos_world + 3 * j, leftankle_pos);

                mju_rotVecMatT_fp(tmp2, tmp, leftfoot_xmat);
                mju_rotVecMatT_fp(conpos_local, tmp2, footRbase);

                num[i] += rightfoot_force[(j + 1) * 3 - 1] * conpos_local[i];

                den[i+3] += rightfoot_force[(j + 1) * 3 - 1]; 

                mju_sub3_fp(tmp, rightfoot_force_pos_world + 3 * j, rightankle_pos);
                mju_rotVecMatT_fp(tmp2, tmp, rightfoot_xmat);
                mju_rotVecMatT_fp(conpos_local, tmp2, footRbase);
                num[i+3] += rightfoot_force[(j + 1) * 3 - 1] * conpos_local[i]; 

            }
            for (int j = 0; j < leftfoot_connum;j++) {
                den[i] += leftfoot_force[(j + 1) * 3 - 1]; //Fz
                
                double tmp[3], tmp2[3], conpos_local[3];
                mju_sub3_fp(tmp, leftfoot_force_pos_world + 3 * j, leftankle_pos);

                //map to the correct frame (base: the foot frame that corresponds to x side, y forward, z upward)
                // base_d = (w_R_foot*foot_R_base)^T*w_d
                mju_rotVecMatT_fp(tmp2, tmp, leftfoot_xmat);
                mju_rotVecMatT_fp(conpos_local, tmp2, footRbase);

                num[i] += leftfoot_force[(j + 1) * 3 - 1] * conpos_local[i];



                den[i+3] += leftfoot_force[(j + 1) * 3 - 1]; 

                mju_sub3_fp(tmp, leftfoot_force_pos_world + 3 * j, rightankle_pos);

                //map to the correct frame (base: the foot frame that corresponds to x side, y forward, z upward)
                // base_d = (w_R_foot*foot_R_base)^T*w_d
                mju_rotVecMatT_fp(tmp2, tmp, rightfoot_xmat);
                mju_rotVecMatT_fp(conpos_local, tmp2, footRbase);
                num[i+3] += leftfoot_force[(j + 1) * 3 - 1] * conpos_local[i];

            }

        }
    } else if(leftfoot_contact && !rightfoot_contact){
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < leftfoot_connum;j++) {
                den[i] += leftfoot_force[(j + 1) * 3 - 1]; //Fz
        ...
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < rightfoot_connum;j++) {
                den[i+3] += rightfoot_force[(j + 1) * 3 - 1]; //Fz
...
void cassie_sim_hold(cassie_sim_t *c)
{
    ...
    for (int i = 0; i < 3; ++i) {
        c->m->jnt_stiffness[i] = 1e7;
        c->m->dof_damping[i] = 1e6;
        c->m->qpos_spring[i] = c->d->qpos[i];
    }

    // Set damping for body rotation joint
    for (int i = 3; i < 6; ++i)
        c->m->dof_damping[i] = 1e7;
}
...
void cassie_sim_hold_horizontal(cassie_sim_t *c, double stif,  double damp)
{
    // Set stiffness/damping for body translation joints
    for (int i = 0; i < 2; ++i) {
        c->m->jnt_stiffness[i] = stif;
        c->m->dof_damping[i] = damp;
        c->m->qpos_spring[i] = c->d->qpos[i];
    }

}

void cassie_sim_release_horizontal(cassie_sim_t *c)
{
    // Zero stiffness/damping for body translation joints
    for (int i = 0; i < 2; ++i) {
        c->m->jnt_stiffness[i] = 0;
...
void cassie_vis_init_recording(cassie_vis_t *sim, const char* videofile, int width, int height, int fps){
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    if (action == GLFW_RELEASE) {
        ...
        if (key == GLFW_KEY_P && mods == 0) {
            printf("attaching camera to pelvis sagittal\n");
            v->cam.type = mjCAMERA_TRACKING;
        ...
        //add a tracking camera for lateral walking
        if (key == GLFW_KEY_L && mods == 0) {
            printf("attaching camera to pelvis lateral\n");
            v->cam.type = mjCAMERA_TRACKING;
            int pel_id = mj_name2id_fp(v->m, mjOBJ_BODY, "cassie-pelvis");
            v->cam.trackbodyid = pel_id;
            v->cam.fixedcamid = -1;
        ...
        //try add a fixed cam at side
        if (key == GLFW_KEY_S && mods == 0) {
            printf("putting camera on the side\n");
            v->cam.type = mjCAMERA_FIXED;
            v->cam.fixedcamid = 0;
        }
        // control keys
        if (mods == GLFW_MOD_CONTROL) {
            if (key == GLFW_KEY_A) {
                memcpy(v->cam.lookat, v->m->stat.center-5, sizeof(v->cam.lookat));
...
cassie_vis_t *cassie_vis_init(cassie_sim_t* c, const char* modelfile, bool offscreen) {
    ...
    //change non-recording video width and height
    v->window = glfwCreateWindow_fp(990, 990, "Cassie", NULL, NULL);
...
bool cassie_vis_draw_largearrow(cassie_vis_t *v, cassie_sim_t *c)
{
