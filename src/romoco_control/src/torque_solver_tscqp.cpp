...
namespace romoco
{
    ...
    BipedMotorCommands TorqueSolverTSCQP::Solve()
    {
        ...
        b_y_ << output_->dJyadq() - output_->d2yd() + KP.cwiseProduct(output_->ya() - output_->yd()) + KD.cwiseProduct(output_->dya() - output_->dyd());
        ...
        if_solved_ = ClarabelSolve(G_, g_, Aub_, bub_, Aeq_, beq_, u_qp);
        Eigen::VectorXd u_full = output_->MapToFullMotors(u_qp);
        ...
        Eigen::VectorXd &sol)
    {
        ...
        if (solution.status == clarabel::SolverStatus::Solved)
        {
            ...
            if (print_qp_)
            {
                ...
                std::cout << "Jh = " << output_->Jh() << std::endl;
