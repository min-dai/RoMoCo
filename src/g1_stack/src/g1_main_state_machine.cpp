...
int main(int argc, char *argv[])
{
    ...
    if (use_estimation)
    {
        ...
        interface = std::make_unique<G1MujocoInterface>(ros_config.config_folder, ros_config.log_path, std::move(robot_ptr_estimation));
    }
    else
    {
        interface = std::make_unique<G1MujocoInterface>(ros_config.config_folder, ros_config.log_path);
    }
